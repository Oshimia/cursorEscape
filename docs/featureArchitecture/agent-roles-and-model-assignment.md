...
Target role catalog for cursorEscape orchestration. Roles are **configurable contracts** ([agents](../agents/_index.md)) — not hardcoded Cursor subagent type strings. Model assignment per role is **Desired** via config; live workflow recommends `composer-2.5` for reviewers ([review-subagent-models](../research/imported/cursor-global-workflow/docs/workflow/review-subagent-models.md)).
...
| **bug_reviewer** | Review | Bugs, security, correctness via openBuggy | Yes (dual gate) |
...
| production_readiness_reviewer | `composer-2.5` class | Config |
| bug_reviewer | Engine default or matched to parent | openBuggy + config |
| implementer | Operator preference | Config |
...
1. **Unknown:** Config schema for role→model→provider triples.
