...
| Deep workflow docs reachable via **absolute companion** Read paths (`{{COMPANION_ROOT}}/workflow/...`, `{{COMPANION_ROOT}}/skills/...`, `{{COMPANION_ROOT}}/agents/...`) from thin harness; **transitional** legacy host `docs/workflow/...` host-root paths (zero `../../` hops) until pointer-first-2 stub rewrite + pointer-first-4 mirror disposition | Any `../../docs|skills|agents` hop authored as if relative to the markdown file (tools resolve from **config root** / cwd — see [Wrong path resolution base](#wrong-path-resolution-base-failure-class--i--j)); treating host `docs/workflow/` mirror as procedure SoT after pointer-first-0 |
...
1. All overlay + live skill Read links / prose paths: **Target** = absolute `{{COMPANION_ROOT}}/workflow/<leaf>.md`; **transitional** (pre pointer-first-2) = host-root `docs/workflow/<leaf>.md` only — **zero** `../../docs/workflow/` in `overlays/opencode/skills/**`.
...
5. **Same class — workflow mirror (2026-08-20 audit; transitional):** `Rewrite-OpenCodeWorkflowLinks.ps1` emitted host-root `skills/...` and `agents/...` — **archived** after pointer-first-0; not primary sync. Overlay `review-subagent-models.md` uses host-root `docs/workflow/...` for workflow leaves until pointer-first-2.
...
| **C4** | Deep workflow Reads resolve on host | Hubs + smoke row **4** How cite absolute `{{COMPANION_ROOT}}/workflow/...` (Target locked pointer-first-0). Harness skill/agent **Read when** bodies still use transitional host-root `docs/workflow/...` until **pointer-first-2** — **zero** `../../docs/workflow/` in overlay harness. Legacy mirror script archived | `implementation-review` skill load; confirm Read resolves `{{COMPANION_ROOT}}/workflow/iterative-code-review.md` (Target How). Legacy `OPENCODE_HOME/docs/workflow/...` pass only while mirror remains transitional | **4** — companion path resolve (locked How in [host-adapter](../SOPs/opencode-host-adapter.md)); plus harness `rg` on overlay tree (author-time Fast CI) |
...
4. Copy-out into `~/.config/opencode` is **authorized and applied** from [overlays/opencode](../../overlays/opencode/_index.md) (Phase 3 live sync 2026-08-20). Runtime smoke rows **1–4**, **9–10**, **13** **pass** (2026-08-20); companion-path row **4** re-probe deferred to pointer-first-2. Cursor copy-out remains manual.
