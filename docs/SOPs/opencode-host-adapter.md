...
| **C3** | Plan→plan_reviewer; impl→dual→Full | 7 overlay agents on disk; reviewers `permission.edit: deny` in agent frontmatter; loops cited in bodies | Rows **2**, **3**, **13** **pass**; row **8** pass; row **14** pass. **Open:** overlay `plan_reviewer` report shape thinner than Cursor portable output schema |
...
7. Smoke **13** gate **pass** (Incomplete until). **Format fidelity gap (Observed):** OpenCode overlay [`plan_reviewer`](../../overlays/opencode/agents/plan_reviewer.md) **Outputs** only require Verdict + Blocking + outstanding changes. Cursor portable [`agents/plan_reviewer.md`](../../agents/plan_reviewer.md) mandates the full schema (Findings summary, Assumptions table, Unknowns, External dependencies, Areas requiring verification, A/B/C, Cost challenge, Failure forecast, Incremental execution, Verification gaps, Architecture alignment, caps/overflow). Short OpenCode reports are primarily **thin overlay authoring**, not smoke failure — weak models amplify the gap. Restore Cursor-parity output format into the overlay agent (or an explicit “load deep output schema” Read) if operator expects the same review shape.
