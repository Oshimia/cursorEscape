...
| Always-on gate text | [`rules/*.md`](../../rules/_index.md) | Gate-body edits land in the **repo-root rule twin** (SoT) and flow to migrated hosts by registry-owned composition — the semantic order lives in `catalog/workflows.json`, not in the migrated manifests. OpenCode `instructions/cursor-escape-loop.md` is **composed** (host `__header__.md` part + authored base body + [`footers/instructions-wiring.md`](../../overlays/opencode/footers/instructions-wiring.md)) with `AGENTS.md` dual-written from the render; Cursor thin `.mdc` hybrid render + matching user-rules-snippet; Antigravity, VS Code, Cline, and Kilo Code always-on gates compose `rules/agent-invocation.md` first, followed by their loop/gate bodies and host wiring; **Antigravity [`GEMINI.md` expected render](../../scripts/host-sync/render-baselines/antigravity/overlays__antigravity__GEMINI.md)** remains the full-replace regression anchor. The five migrated host groups use `CompositionId` (never `Parts`/`Footer`) for registry-governed order; VS Code currently keeps authoritative manifest-owned `Parts`/`Footer`. The blocking `composition-order-ownership` guard in normalization Fast CI rejects divergence on `CompositionId` entries |
...
| Composed-gate wiring (`CompositionId` / `base:`/`shared:` classes) | [`catalog/workflows.json`](../../catalog/workflows.json) (semantic order for migrated groups) + [`scripts/host-sync/manifests/*.psd1`](../../scripts/host-sync/) (destinations plus `CompositionId`; VS Code keeps current `Parts`/`Footer`) + the referenced part/footer leaves | FA recording ([skill-source-and-host-overlays](../featureArchitecture/skill-source-and-host-overlays.md)); [host-sync README](../../scripts/host-sync/README.md); render-baselines under [`scripts/host-sync/render-baselines/`](../../scripts/host-sync/render-baselines/) re-captured only from observed current renders; normalization Fast CI validates blocking ownership |
...
### Live installs (owner-authorized only)
...
| Antigravity | [`overlays/antigravity/`](../../overlays/antigravity/_index.md) | included automatically; Apply requires fresh owner authorization (see [Apply boundary](../featureArchitecture/procedure-registry.md)) |
| VS Code | [`overlays/vscode/`](../../overlays/vscode/_index.md) | included automatically (`~/.copilot`); Apply requires fresh owner authorization; see [vscode-host-adapter](./vscode-host-adapter.md) |
| Cline | [`overlays/cline/`](../../overlays/cline/_index.md) | included automatically (`~/.cline`); Apply requires fresh owner authorization; see [cline-host-adapter](./cline-host-adapter.md) |
| Kilo Code | [`overlays/kilocode/`](../../overlays/kilocode/_index.md) | included automatically (`~/.kilocode`); Apply requires fresh owner authorization; see [kilocode-host-adapter](./kilocode-host-adapter.md) |
| Codex | [`overlays/codex/`](../../overlays/codex/_index.md) | registered and included in all-stack planning/CI; Apply requires fresh owner authorization; see [codex-host-adapter](./codex-host-adapter.md) |
...
**Live Apply requires fresh explicit owner authorization.** Dry-run (`pwsh ./scripts/Sync-HostHarness.ps1`) is always allowed. No normalization or review step writes to live hosts.
