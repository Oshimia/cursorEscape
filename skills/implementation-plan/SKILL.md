...
- If Escalation=yes: Agent context per [plan-agent-context.md](../../workflow/plan-agent-context.md) (including Inter-phase / Migration when applicable)

Do **not** invent fixed always-on line/character budgets in Success, Verification, or phase wording.

This section is the **sole SoT** for the plan section checklist. [iterative-plan-review.md](../../workflow/iterative-plan-review.md) and [plan_reviewer](../../agents/plan_reviewer.md) require compliance — they must **not** paste a second full enum.
...
Mandatory on every non-trivial plan. Place **immediately after Scope**. Schema and Agent context headings: [plan-agent-context.md](../../workflow/plan-agent-context.md).
...
If unsure Escalation=yes: ask the user; do not silently escalate. Escalation **yes** requires Inter-phase contracts, Migration/external apply order (or **none**), and full Agent context blocks per [plan-agent-context.md](../../workflow/plan-agent-context.md).
...
- **`#### Agent context — Phase N`** for every execution phase — required headings in [plan-agent-context.md](../../workflow/plan-agent-context.md) (link; do not paste the full specimen into this skill)
...
| Escalated Agent context / contracts | Escalation **yes** → [plan-agent-context.md](../../workflow/plan-agent-context.md) headings |
...
When Escalation is **yes**, put full Agent context (and contracts) **in the accepted plan** per [plan-agent-context.md](../../workflow/plan-agent-context.md) — not only after accept in a repo roadmap.

After approval, use the [`roadmap`](../roadmap/SKILL.md) skill to write a **repo** roadmap: Escalation **yes** → **copy** from the plan (fail-closed if stub); Escalation **no** → optional roadmap that **may restructure** thin Incremental execution into Agent context **without new scope**. See [phased-multi-agent.md](../../workflow/phased-multi-agent.md). Do not store roadmaps under `~/.cursor`. Prefer [`documentation-architecture`](../documentation-architecture/SKILL.md) when bootstrapping new docs areas.
