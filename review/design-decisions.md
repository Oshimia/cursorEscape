...
| **Primary job** | **Manage skills and workflows that work** — plan, implement, dual review, repository discovery — as portable contracts in this repo, applied across stacks; OpenCode host-plugged copy-out **authorized** from [overlays/opencode](../overlays/opencode/_index.md) (Phase 2); Cursor copy-out remains manual. |
| **Canonical skill tree (Target — Approach A)** | Portable procedure and contracts at repo-root `workflow/`, `skills/`, `agents/`, `rules/` (Phases 3–5). Host dirs are **copy-out targets**, not a second authored tree. OpenCode overlay at `overlays/opencode/`; Cursor thin wrappers at `overlays/cursor/`. OpenCode live sync Phase 3; Cursor copy-out not repo-authorized. |
