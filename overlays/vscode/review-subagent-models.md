...
VS Code note: `.agent.md` supports a `model` frontmatter field (single name or fallback array). Models are host-config-specific and owner-controlled BYOK; the vscode overlay leaves `model` unset so the chat picker default applies. Set per-agent explicitly only when the owner assigns one.
