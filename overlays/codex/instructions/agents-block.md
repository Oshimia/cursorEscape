...
OpenAI Codex adapter for the owner's agentic loop. Canonical procedure is companion-resident under `{{COMPANION_ROOT}}/skills/`, `{{COMPANION_ROOT}}/workflow/`, `{{COMPANION_ROOT}}/agents/`, and `{{COMPANION_ROOT}}/rules/`; the installed harness advertises and points, it is not a second procedure tree. When asked to quote default-on, when-in-doubt, or eval/harness gates, answer from this block without searching for the policy.
