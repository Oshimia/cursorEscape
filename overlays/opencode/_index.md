...
OpenCode-native **host overlay** at `overlays/opencode/`. Portable procedure stays at repo-root bases (`workflow/`, `skills/`, `agents/`, `rules/`). Overlay = **thin harness** + host-native agent bodies. Live `~/.config/opencode` harness synced via [`Sync-HostHarness.ps1`](../../scripts/Sync-HostHarness.ps1) — dry-run by default (add `-Target OpenCode` for the planned dry-run view); live `-Apply` requires explicit owner authorization, the normative live path is all-host `-Apply -Target All`, and single-stack `-Apply` is only an explicitly owner-authorized `-AllowSkew` recovery exception — see [opencode-host-adapter](../../docs/SOPs/opencode-host-adapter.md) and [host-sync README](../../scripts/host-sync/README.md). **pointer-first-4:** procedure mirror **deleted** from live OpenCode; harness-only sync only.
...
**Status (pointer-first-0):** [Rewrite-OpenCodeWorkflowLinks.ps1](./scripts/Rewrite-OpenCodeWorkflowLinks.ps1) is **archived** — used for Phase 3 historical mirror only. **Not** the Target load path after pointer-first-0. **Target load path (pointer-first-2):** harness stub bodies Read `{{COMPANION_ROOT}}/workflow/<leaf>.md` directly. Mirror delete proof: live `{{OPENCODE_HOME}}/docs/workflow/` procedure mirror deleted — see Mirror disposition above.
...
## Workflow leaf manifest (ARCHIVED — Phase 3 baseline; mirror deleted pf4)

**Status:** Historical record only. Live OpenCode `docs/workflow/` mirror **deleted** pointer-first-4. Do not re-sync from this table.

Companion `workflow/` leaves formerly mirrored to host `docs/workflow/` (**10** files):
...
| 9 | *(overlay — pointer-only pf4)* | ~~`docs/workflow/review-subagent-models.md`~~ — companion `overlays/opencode/review-subagent-models.md` |
