---
description: Launch cursorEscape governed-agent fallback routes in separate fresh Kilo task/session legs
---

Until Kilo subtask isolation is attested for cursorEscape governed roles, use a **separate fresh Kilo task/session per governed leg**. Never convert a role by changing personas in an existing conversation; a reused conversation is a routing/isolation failure, not a valid governed result.
...
Apply each role's role-specific required reading from `{{COMPANION_ROOT}}/workflow/agent-invocation.md`. The route identity is the canonical identity; no Kilo command, mode, or alias replaces it.

## `implementer` fresh-task envelope

Start a new Kilo task/session. Begin its payload exactly with this envelope before task-specific context:
...
Close or set aside the implementation leg, then start another new Kilo task/session. Begin its payload exactly with this envelope before parent-supplied test context:
