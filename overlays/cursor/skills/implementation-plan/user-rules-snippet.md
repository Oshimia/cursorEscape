...
2. Invoke plan-reviewer (max 3 passes), clean context, full synthesized plan only. The child prompt must begin exactly with the canonical envelope from workflow/agent-invocation.md; host alias never replaces canonical identity. Recommended model: composer-2.5.
